...
public class Bot implements IBot {
    ...
    public Bot(World world) {
        ...
        health = 5;
        alive = LV_ALIVE;
        //Class[] parameterTypes = new Class[] { ru.cyberbiology.Bot.class};
...
//.......  пришло время подохнуть или породить потомка              ........
    ...
    @Override
    public void relink(IBot[][] matrix) {
        if(this.mprevX>-1 && this.mprevY>-1)
        {
            this.mprev	= (Bot) matrix[this.mprevX][this.mprevY];
        }
        if(this.mnextX>-1 && this.mnextY>-1)
        {
            this.mnext	= (Bot) matrix[this.mnextX][this.mnextY];
        }
    }
...
//        System.out.println(world.generation + ": " + bot.health + " + " + hlt);
    ...
    public int botMove(Bot bot, int direction, int ra) { // ссылка на бота, направлелие и флажок(относительное или абсолютное направление)
        ...
        if (world.matrix[xt][yt].getAlive() <= LV_ORGANIC_SINK) { // если на клетке находится органика
            return 4;                       // то возвращаем 4
        }
        if (isRelative(bot, (Bot) world.matrix[xt][yt]) == 1) {  // если на клетке родня
    ...
    int botEat(Bot bot, int direction, int ra) { // на входе ссылка на бота, направлелие и флажок(относительное или абсолютное направление)
        ...
        // осталось 2 варианта: ограника или бот
        else if (world.matrix[xt][yt].getAlive() <= LV_ORGANIC_SINK) {   // если там оказалась органика
            deleteBot((Bot) world.matrix[xt][yt]);                           // то удаляем её из списков
        ...
        int min1 = ((Bot) world.matrix[xt][yt]).mineral;  // определим количество минералов у потенциального обеда
        int hl = ((Bot) world.matrix[xt][yt]).health;  // определим энергию у потенциального обеда
        ...
        if (min0 >= min1) {
            ...
            deleteBot((Bot) world.matrix[xt][yt]);          // удаляем жертву из списков
        ...
        ((Bot) world.matrix[xt][yt]).mineral = min1 - min0;       // перезаписали минералы жертве =========================ЗАПЛАТКА!!!!!!!!!!!!
        ...
        ((Bot) world.matrix[xt][yt]).mineral = min1 - (bot.health / 2);  // у жертвы минералы истраченны
    ...
    int botSeeBots(Bot bot, int direction, int ra) { // на входе ссылка на бота, направлелие и флажок(относительное или абсолютное направление)
        ...
        if (yt < 0 || yt >= world.height) {  // если там стена возвращаем 3
            ...
            return 2;
        } else if (world.matrix[xt][yt].getAlive() <= LV_ORGANIC_SINK) { // если органика возвращаем 4
            return 4;
        } else if (isRelative(bot, (Bot) world.matrix[xt][yt]) == 1) {  // если родня, то возвращаем 6
            return 6;
    ...
    void botGenAttack(Bot bot) {   // вычисляем кто у нас перед ботом (используется только относительное направление вперед)
        ...
        if ((yt >= 0) && (yt < world.height) && (world.matrix[xt][yt] != null)) {
            if (world.matrix[xt][yt].getAlive() == LV_ALIVE) { // если там живой бот
                ...
                if (bot.health > 0) {                    // если он при этом не умер
                    ...
                    ((Bot) world.matrix[xt][yt]).mind[ma] = mc;
                }
            }
        }
    }
    ...
    int botCare(Bot bot, int direction, int ra) { // на входе ссылка на бота, направлелие и флажок(относительное или абсолютное направление)
        ...
        int hlt1 = ((Bot) world.matrix[xt][yt]).health;  // у бота и его соседа
        int min0 = bot.mineral;
        int min1 = ((Bot) world.matrix[xt][yt]).mineral;
        if (hlt0 > hlt1) {              // если у бота больше энергии, чем у соседа
            ...
            ((Bot) world.matrix[xt][yt]).health = ((Bot) world.matrix[xt][yt]).health + hlt;
        }
        if (min0 > min1) {              // если у бота больше минералов, чем у соседа
            ...
            ((Bot) world.matrix[xt][yt]).mineral = ((Bot) world.matrix[xt][yt]).mineral + min;
        }
        return 5;
    }
    ...
    {                         // на выходе стена - 2 пусто - 3 органика - 4 удачно - 5
        ...
        if (min0 > 3) {                 // только если их у него не меньше 4
            ...
            ((Bot) world.matrix[xt][yt]).mineral = ((Bot) world.matrix[xt][yt]).mineral + min;
            if (((Bot) world.matrix[xt][yt]).mineral > 999) {
                ((Bot) world.matrix[xt][yt]).mineral = 999;
            }
        }
        return 5;
    }
